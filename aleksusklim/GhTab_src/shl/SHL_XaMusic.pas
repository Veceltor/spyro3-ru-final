unit SHL_XaMusic; // SpyroHackingLib is licensed under WTFPL

interface

uses
  SysUtils, Classes, SHL_IsoReader, SHL_Types;
...
type
  ...
  private
    ...
    FVolume: Double;
  public
    class function TrackInfoByXa(XaHeader: Integer; out Channel: Byte; out
      Stereo: Boolean; out HiQuality: Boolean; out MinStride: Byte): Boolean; overload;
    class function TrackInfoByXa(XaHeader: Pointer): Boolean; overload;
    class function DecodeXaData(SectorData: Pointer; SaveWave: Pointer; InStereo:
      Boolean; OutStereo: Boolean; SpecialStuff: Pointer; Volume: Double = 1.0): Boolean;
    class procedure XaEmprySector(WriteBodyTo: Pointer);
  public
    function Start(Image: TIsoReader; Quality, InStereo, OutStereo: Boolean;
      Sector, Stride: Integer; Count: Integer = 0; Volume: Double = 1.0): Boolean;
...
function TXaDecoder.Start(Image: TIsoReader; Quality, InStereo, OutStereo:
  Boolean; Sector, Stride: Integer; Count: Integer = 0; Volume: Double = 1.0): Boolean;
begin
  ZeroMem(@FExtra[0], 32);
  FImage := Image;
  FQuality := Quality;
...
begin
  ...
  ZeroMem(@Buffer[0], 3000);
  FImage.ReadSectors(@Buffer[0]);
  FImage.SeekToSector(Next);
  Result := DecodeXaData(@Buffer[8], WaveOut, FInStereo, FOutStereo, @FExtra[0], Volume);
end;
...
function TXaDecoder.Write(const Buffer; Count: Integer): Integer;
begin
  Ignore(Buffer, Count);
  Result := 0;
end;

function TXaDecoder.Read(var Buffer; Count: Integer): Integer;
var
  Have: Integer;
  Save: Pointer;
begin
  ...
  begin
    ...
    if Count < Have then
      Have := Count;
    CopyMem(Cast(FBuffer, FPos), Save, Have);
    Inc(FPos, Have);
    Adv(Save, Have);
    Inc(Result, Have);
    Dec(Count, Have);
...
function TXaDecoder.Seek(Offset: Integer; Origin: Word): Integer;
begin
  Ignore(Offset, Origin);
  Result := 0;
end;

class function TXaDecoder.TrackInfoByXa(XaHeader: Integer; out Channel: Byte;
  out Stereo: Boolean; out HiQuality: Boolean; out MinStride: Byte): Boolean;
begin
  Result := False;
  Channel := 255;
  if XaHeader = -1 then
...
class function TXaDecoder.DecodeXaData(SectorData: Pointer; SaveWave: Pointer;
  InStereo: Boolean; OutStereo: Boolean; SpecialStuff: Pointer; Volume: Double =
  1.0): Boolean;
const
  k0: array[0..3] of Double = (0.0, 0.9375, 1.796875, 1.53125);
  k1: array[0..3] of Double = (0.0, 0.0, -0.8125, -0.859375);
var
  Data: Pointer;
  Save: PSmallInt;
  Stuff: PDouble;
...
begin
  ...
  begin
    ...
    while Cnt < 8 do
    begin
      Temp := CastByte(Data, 4 + Cnt)^;
      r[0] := Temp and 15;
      f[0] := (Temp shr 4) and 3;
      if InStereo then
      begin
        Temp := CastByte(Data, 4 + Cnt + 1)^;
        r[1] := Temp and 15;
      ...
      begin
        ...
        begin
          a := 1 shl (12 - r[Sec]);
          Temp := (CastByte(Data, 16 + ((Cnt + Sec) shr 1) + (Index shl 2))^ shr
            (((Cnt + Sec) and 1) shl 2)) and 15;
          if Temp > 7 then
            b := (Temp - 16) * a
          else
            b := Temp * a;
