...
type
  ...
  public
    ...
    function SaveTo(Memory: Pointer; Size: Integer): Boolean; overload;
...
begin
  ...
  if (Self = nil) or (FMemory = nil) then
    Exit;
  ZeroMem(FMemory, FWidth * FHeight * 2);
  Result := True;
end;
...
begin
  ...
  if Result then
    CopyMem(From.FMemory, FMemory, FWidth * FHeight * 2);
end;
...
begin
  ...
  for Index := 0 to FHeight - 1 do
    CopyMem(From.Cell(X, Y + Index), Cell(0, Index), Count);
end;
...
begin
  ...
  for Index := 0 to From.FHeight - 1 do
    CopyMem(From.Cell(0, Index), Cell(X, Y + Index), Count);
end;
...
begin
  ...
  try
    ...
    if Size <> Need then
    begin
      ZeroMem(Cast(FMemory, Size), Need - Size);
      Exit;
    end;
  except
    Exit;
...
begin
  ...
  try
    if Size >= Need then
      CopyMem(Memory, FMemory, Need)
    else
    begin
      CopyMem(Memory, FMemory, Size);
      ZeroMem(Cast(FMemory, Size), Need - Size);
      Exit;
    end;
  except
...
function TVram.SaveTo(Memory: Pointer; Size: Integer): Boolean;
var
  Mem: Pointer;
  Len: Integer;
begin
  Result := False;
  Mem := RawData(Len);
  if (Mem = nil) or (Size < Len) then
    Exit;
  try
    CopyMem(Mem, Memory, Len);
    Result := True;
  except
  end;
end;
