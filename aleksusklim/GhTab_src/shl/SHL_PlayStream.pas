unit SHL_PlayStream; // SpyroHackingLib is licensed under WTFPL

interface

uses
  Windows, MMSystem, SysUtils, Classes, Contnrs, SHL_WaveStream, SHL_Types;
...
type
  ...
  public
   // stream can be instantiated by default Create(); this constructor will call Open:
    constructor Create(Device: Integer; Samples: Integer; IsStereo: Boolean;
      Is16Bit: Boolean = True; Limit: Integer = 4); overload;
   // get all settings from a TWaveStream:
    constructor Create(Device: Integer; WaveStream: TWaveStream; Limit: Integer
      = 4); overload;
  ...
  public
    ...
    function Open(Device: Integer; Samples: Integer; IsStereo: Boolean; Is16Bit:
      Boolean = True; Limit: Integer = 4): Boolean; overload;
    // for second constructor:
    function Open(Device: Integer; WaveStream: TWaveStream; Limit: Integer = 4):
      Boolean; overload;
    ...
    procedure Dump(Filename: TextString = '');
...
constructor TPlayStream.Create(Device: Integer; Samples: Integer; IsStereo:
  Boolean; Is16Bit: Boolean = True; Limit: Integer = 4);
begin
  inherited Create();
...
constructor TPlayStream.Create(Device: Integer; WaveStream: TWaveStream; Limit:
  Integer = 4);
begin
  inherited Create();
...
function TPlayStream.Open(Device: Integer; Samples: Integer; IsStereo: Boolean;
  Is16Bit: Boolean = True; Limit: Integer = 4): Boolean;
var
...
begin
  ...
  // get handle:
  Result := (waveOutOpen(@FDevice, Device, @Format, FEvent, 0, CALLBACK_EVENT) =
    MMSYSERR_NOERROR) and (FDevice <> 0);
end;

function TPlayStream.Open(Device: Integer; WaveStream: TWaveStream; Limit:
  Integer = 4): Boolean;
begin
  Result := Open(Device, WaveStream.SampleRate, WaveStream.IsStereo, WaveStream.Is16Bit,
    Limit);
end;
...
begin
  ...
  ZeroMem(Header, SizeOf(TWaveHdr));
  Header.lpData := PDataChar_(Header) + SizeOf(TWaveHdr); // here
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Desten^ := (((((Value and $ffff) shr FVolume) xor Mask) - Mask) and
            $ffff) or ((((Value shr 16) shr FVolume) xor Mask) - Mask) shl 16;
          Inc(Source);
          Inc(Desten);
          Dec(Total);
        end
      ...
      begin // 8-bit algo
        ...
        Value := Integer($80808080) - (Integer($80808080) shr FVolume); // difference to center the value
        while Total <> 0 do
        begin
      ...
      ZeroMem(Header.lpData, SizeInBytes);
  end
  else
    CopyMem(WaveData, Header.lpData, SizeInBytes); // just copy samples
...
procedure TPlayStream.Dump(Filename: TextString = '');
begin
  if Filename = '' then
  begin
    if FDump = nil then
...
function TPlayStream.Write(const Buffer; Count: Integer): Integer;
begin
...
function TPlayStream.GetPos(): Integer;
var
  Time: tMMTIME;
begin
...
function TPlayStream.Seek(Offset: Integer; Origin: Word): Integer;
begin
  Ignore(Offset, Origin);
  Result := 0; // impossible
end;

function TPlayStream.Read(var Buffer; Count: Integer): Integer;
begin
  Ignore(Buffer, Count);
  Result := 0; // impossible
end;
