...
uses
  ...
  SHL_TextUtils, SHL_TextureManager, SHL_Models3D, SHL_Types;
...
type
  ...
  private
    ...
    FCacheVertex: array[0..255] of RVertex;
    FCacheColors: array[0..255] of RColor;
    FPreparedFrame: Integer;
    FTiangle: Boolean;
  ...
  public
    ...
    procedure SaveObj(Obj: TObjModel; Model: Integer; TextureManager:
      TTextureManager = nil);
...
begin
  ...
  CopyMem(Cast(Model, 2), Cast(Result.Data), Count);

  Count := Gold.GetVertexCount(Model) * 4;
  SetLength(Result.Vertex, Count);
  CopyMem(Cast(Model, 16), Cast(Result.Vertex), Count);

  Count := Gold.GetColorCount(Model) * 4;
  SetLength(Result.Colors, Count * 2);
  CopyMem(Gold.GetColor(Model, 0, False), Cast(Result.Colors), Count);
  CopyMem(Gold.GetColor(Model, 0, True), Cast(Result.Colors, Count), Count);

  Data := Gold.GetPoly(Model, Count, False);
  SetLength(Result.Poly, Count);
  CopyMem(Data, Cast(Result.Poly), Count);

  Data := Gold.GetPoly(Model, Count, True);
  SetLength(Result.Low, Count);
  CopyMem(Data, Cast(Result.Low), Count);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      PChunk^ := Len;
      Inc(PChunk);
      CopyMem(Cast(Model.Poly), PChunk, Len);
      ...
      PChunk^ := Len;
      Inc(PChunk);
      CopyMem(Cast(Model.Low), PChunk, Len);
      Inc(PChunk, Len div 4);
      ...
      begin
        ...
        CopyMem(Cast(Model.Colors), PChunk, Len);
...
procedure TGoldenFont.VertexStart();
begin
...
function TGoldenFont.Triangle(V1, V2, V3, V4: Integer; out A1, A2, A3, B1, B2,
  B3, Q1, Q2, Q3, Q4: Integer; TwoSideFlag: Boolean): Boolean; // TwoSideFlag??
begin
//  4 - 2
//  | / |
//  3 - 1
...
function TGoldenFont.Triangle(V1, V2, V3, V4: Integer; out A1, A2, A3, B1, B2,
  B3, Q1, Q2, Q3, Q4: Integer; TwoSizeFlag: Boolean): Boolean; // TwoSizeFlag??
begin
//  4 - 2
//  | / |
//  3 - 1
...
begin
  ...
  begin
    ...
    TwoSizeFlag := False;
  ZeroMem(@TiangleOne, SizeOf(TiangleOne));
  ZeroMem(@TiangleTwo, SizeOf(TiangleTwo));
...
begin
  ...
  begin
    Inc(Result);
    Obj.AddVertex(SModels3D.Vertex2Point(Vertex));
  end;
end;
...
begin
  ...
  begin
    Inc(Result);
    ZeroMem(@Quad, SizeOf(Quad));
    ZeroMem(@VramTexture, SizeOf(VramTexture));
    Quad.Vertex := Vertex;
...
procedure TGoldenFont.SaveObj(Obj: TObjModel; Model: Integer; TextureManager:
  TTextureManager = nil);
var
  Quad: RPolyQuad;
  Tria: array[1..2] of RPolyQuad;
  VramTexture: RVramTexture;
  HaveTexture, HaveNormal: Boolean;
  HaveAlpha: Boolean;
  Norm: RPoint;
  Index, Frame: Integer;
  TextureQuad: RTextureQuad;
...
const
  Flag = 0;
begin
//  UseModel(Model, 0, False);
  for Frame := 0 to ModelFrames(Model) - 1 do
  begin
    UseModel(Model, Frame, False);
    PrepareModel(Frame);
//    PrepareModel(FUsedFrame);
    Obj.AddGroup('fr_' + STextUtils.IntToStrPad(Frame, 2));
    FaceStart();
    ...
    begin
      ...
      if HaveNormal then
      begin
        ZeroMem(@Quad.Color, SizeOf(Quad.Color));
      end
      else
        FaceColor(Quad.Color);
      if HaveTexture then
  ...
  //    if HaveAlpha then
    {  begin
        ...
        begin
          if HaveNormal then
            Obj.AddVertexColor(FCacheVertex[Tria[Count].Vertex[Index]].X,
              FCacheVertex[Tria[Count].Vertex[Index]].Y, FCacheVertex[Tria[Count].Vertex
              [Index]].Z, Col.R, Col.G, Col.B)
          else
            Obj.AddVertexColor(FCacheVertex[Tria[Count].Vertex[Index]].X,
              FCacheVertex[Tria[Count].Vertex[Index]].Y, FCacheVertex[Tria[Count].Vertex
              [Index]].Z, FCacheColors[Tria[Count].Color[Index]].R, FCacheColors
              [Tria[Count].Color[Index]].G, FCacheColors[Tria[Count].Color[Index]].B);
          if HaveTexture then
            Obj.AddTexture(Texts[Tria[Count].Texture[Index]].X, Texts[Tria[Count].Texture
              [Index]].Y, 0);
          if HaveNormal then
        ...
        begin
          if HaveNormal then
            Obj.AddFaceTextureNormal(-3, -3, -3, -2, -2, -2, -1, -1, -1)
          else
            Obj.AddFaceTexture(-3, -3, -2, -2, -1, -1);
        end
        else
        begin
          if HaveNormal then
            Obj.AddFaceNormal(-3, -3, -2, -2, -1, -1)
          else
            Obj.AddFace(-3, -2, -1);
        end;
        Dec(Count);
...
procedure TGoldenFont.MayPrepareModel();
begin
  FPreparedFrame := -1;
  PrepareModel(FUsedFrame);
end;
...
var
  ...
  First: Boolean;
begin
  if Frame = FPreparedFrame then
    Exit;
  FPreparedFrame := Frame;
  FillChar(FCacheColors, SizeOf(FCacheColors[0]) * Length(FCacheColors), 127);
  FillChar(FCacheVertex, SizeOf(FCacheVertex[0]) * Length(FCacheVertex), 0);
//  SetLength(FCacheVertex, 0);
  First := True;
//  Count := 0;
//  VertexStart();
