...
type
  ...
  AJap022Data = array of RJap022Data;
...
//    class function CutSubfileImportTracks(const Datas: ALevelData): ALevelPart;
  ...
  public
    ...
    function UseLevelSub(const Subfile: DataString): Boolean; overload;
    ...
    function LevelGetSublevelVram(Index: Integer; Vram: TVram; out X, Y: Integer):
      Boolean;
    function LevelGetSublevelText(SubLevel: Integer; out Size: Integer): Pointer;
    function Jap022Load(Data: DataString): AJap022Data;
    function Jap022Save(Data: AJap022Data): DataString;
  public
    property Game: NGame read FGame;
  end;
...
begin
  ...
  CopyMem(Subfile, FLevelCopy, 2048);
  Result := True;
  FLevelSub := Subfile;
  FLevelSize := Size;
  GuessGame();
...
function TWadManager.UseLevelSub(const Subfile: DataString): Boolean;
begin
...
begin
  if Size <> 0 then
  begin
    CopyMem(Addr, Cast(FLevelSub, Offset), Size);
    Inc(Offset, Size);
  end;
end;
...
begin
  ...
  begin
    ...
    ZeroMem(Cast(FLevelSub, Offset), Size);
    Inc(Offset, Size);
  end;
end;
...
begin
  ...
  ZeroMem(Target, Result);
  if Source = nil then
    Exit;
  if Result < Size then
    Size := Result;
  CopyMem(Source, Target, Size);
end;
...
begin
  ...
  try
    ...
    CopyMem(Part.Addr, FLevelSub, Part.Size);
    GuessGame();
    ZeroMem(FLevelSub, Part.Size);
    Offset := Part.Size;
    PushZero(Offset);
    case FGame of
      GameSpyro1:
...
begin
  ...
  try
    ...
    Assure(Part.Size = 12);
    CopyMem(Part.Addr, From, Part.Size);
    Data := Pointer(From);
    Inc(Data);
    Blocks := Data^;
    Inc(Data);
...
function TWadManager.LevelGetSublevelText(SubLevel: Integer; out Size: Integer): Pointer;
begin
  Result := nil;
  if (SubLevel = 1) or (FGame = GameSpyro3) then
    Result := GetSubsubfile(SublevelTextToSubsubfile(SubLevel), Size);
end;

function TWadManager.Jap022Load(Data: DataString): AJap022Data;
var
  Step: Pointer;
  Size, Count, Index, Last: Integer;
...
begin
  ...
  Step := Cast(Data);
  Size := CastInt(Step)^;
  if (Size < 8) or (Size > Length(Data)) then
    Exit;
  Count := 0;
  Last := (Size - 8) div 8;
  Adv(Step, 4);
  while Count < Last do
  begin
    if CastInt(Step, Count * 8)^ = 0 then
      Break;
    Inc(Count);
  end;
  ...
  SetLength(From, Count + 1);
  for Index := 0 to Count - 1 do
  begin
    Last := CastInt(Step)^;
    Adv(Step, 4);
    Result[Index].Flag := CastInt(Step)^;
    Adv(Step, 4);
    if (Index > 0) and ((Last < From[Index - 1]) or (Last >= Size)) then
    begin
      Count := 0;
      Break;
    end;
  ...
  for Index := 0 to Count - 1 do
    SetString(Result[Index].Data, CastChar(Data, From[Index]), From[Index + 1] -
      From[Index]);
end;

function TWadManager.Jap022Save(Data: AJap022Data): DataString;
var
  Step: Pointer;
  Len, Size, Index: Integer;
  Head: DataString;
begin
  Result := '';
  Len := Length(Data);
  if Len = 0 then
    Exit;
  Size := AlignValue(Len * 8 + 4, 2048);
  SetLength(Head, Size);
  Step := Cast(Head);
  ZeroMem(Step, Size);
  for Index := 0 to Len - 1 do
  begin
    CastInt(Adv(Step, 4))^ := Size;
    CastInt(Adv(Step, 4))^ := Data[Index].Flag;
    Inc(Size, Length(Data[Index].Data));
  end;
  SetLength(Result, AlignValue(Size, 2048));
  Step := Cast(Result);
  CastInt(Step)^ := Size;
  CopyMem(Cast(Head), Step, Length(Head));
  Adv(Step, Length(Head));
  for Index := 0 to Len - 1 do
  begin
    CopyMem(Cast(Data[Index].Data), Step, Length(Data[Index].Data));
    Adv(Step, Length(Data[Index].Data));
  end;
  Len := Diff(Step, Result);
  CastInt(Result)^ := Len;
  ZeroMem(Step, Size - Len);
end;
